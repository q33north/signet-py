...
class Researcher:
    ...
    async def _pick_topic(self) -> tuple[str, str]:
        ...
        if queued:
            ...
            await self._research.consume_queue_item(
                queue_id, research_id=ResearchArtifact(topic=topic).id
            )
            return topic, angle
    ...
    async def _gather_candidates(self) -> list[str]:
        ...
        try:
            ...
            for r in dream_results:
                if r.dream.entity_name:
                    candidates.append(
                        f"{r.dream.entity_name}: {r.dream.content[:100]}"
                    )
        except Exception:
        ...
        try:
            ...
            for r in wiki_results:
                candidates.append(
                    f"Extend knowledge on: {r.article.frontmatter.title}"
                )
        except Exception:
        ...
        try:
            ...
            for r in wiki_results:
                parts.append(
                    ...
                    f"{r.article.frontmatter.summary or r.article.body[:200]}"
                )
        except Exception:
            pass
    ...
    async def _synthesize(self, artifact: ResearchArtifact) -> ResearchArtifact:
        ...
        try:
            data = _parse_json(raw)
            artifact.synthesis = data.get("synthesis", raw)
            artifact.confidence = data.get("confidence", "medium")
            artifact.open_questions = data.get("open_questions", [])
            artifact.suggested_next = data.get("suggested_next", [])
        except (json.JSONDecodeError, KeyError):
            # Use raw response as synthesis
            artifact.synthesis = raw
